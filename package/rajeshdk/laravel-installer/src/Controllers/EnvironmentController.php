...
class EnvironmentController extends Controller
{
    ...
    private function checkValid(Request $request)
    {
        ...
        try {
            ...
            $options = [
                'form_params' => [
                    "c" => 'nullmasterinbabiato',
                    "mail" => 'nullmaster@babiato.org',
                    "u" => url('/'),
                ]
            ];
         ...
         try {
           ...
           return array('status' => 'Error', 'message' => trans('installer_messages.environment.wizard.form.db_communication_failed').$e->getMessage());
            ...
            try {
  			...
  			$sql = base_path('rose_sql.sql');
			 //$conn->unprepared(file_get_contents(base_path() .'/pos.sql'));
			 $conn->unprepared(file_get_contents($sql));                    

 //$conn->unprepared(file_get_contents(base_path() .'/pos.sql'));
					 $conn->unprepared(file_get_contents('database/dump/pos.sql'));
                    $conn->commit();
                    return true;
